...
void send_initial_clock_train (void)        //EDITED
{
    ...
    EALLOW;
    GpioCtrlRegs.GPCLOCK.bit.GPIO69 = 0;
    GpioCtrlRegs.GPCMUX1.bit.GPIO69 = 0;
    GpioCtrlRegs.GPCDIR.bit.GPIO69 = 1;
    GpioCtrlRegs.GPCLOCK.bit.GPIO69 = 1;
    EDIS;
...
void power_on (void)    //EDITED
{
    ...
    GpioCtrlRegs.GPCLOCK.bit.GPIO69 = 0;
    GpioCtrlRegs.GPCLOCK.bit.GPIO70 = 0;
    GpioCtrlRegs.GPCLOCK.bit.GPIO71 = 0;
    GpioCtrlRegs.GPCLOCK.bit.GPIO72 = 0;

    //Set up  MUX & DIR
    GpioCtrlRegs.GPCMUX1.bit.GPIO72 = 3;    //Leave as GPIO for manual CS control No!
    ...
    GpioCtrlRegs.GPCCSEL1.bit.GPIO69 = 0;
    GpioCtrlRegs.GPCCSEL1.bit.GPIO70 = 0;
    GpioCtrlRegs.GPCCSEL1.bit.GPIO71 = 0;
    GpioCtrlRegs.GPCCSEL2.bit.GPIO72 = 0;

    //Lock the SD-Card SPI GPIOs
    GpioCtrlRegs.GPCLOCK.bit.GPIO69 = 1;
    GpioCtrlRegs.GPCLOCK.bit.GPIO70 = 1;
    GpioCtrlRegs.GPCLOCK.bit.GPIO71 = 1;
    GpioCtrlRegs.GPCLOCK.bit.GPIO72 = 1;
...
void set_max_speed(void)            //EDIT
{
    ...
    SpicRegs.SPIBRR.all = 3; //50Mbps는 현재 가지고 있는 MicroSD카드에서는 불가함... //최대 25Mbps 가능... Non highSpeed모드에 비해 속도 2배 향상!
    //삼성의 32GB SDHC PRO를 사용해봤지만 50Mbps로 통신 불가... 25Mbps가 한계인 듯 하다(90Mbps Write 지원)
    /*********************/
...
//                xmit_spi(*buff);
//                buff++;
                a = (BYTE)__byte(buff,0);
                xmit_spi(a);

                a = (BYTE)__byte(buff,1);
                xmit_spi(a);
                buff++;
            }
            while (--wc);
