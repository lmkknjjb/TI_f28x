...
/---------------------------------------------------------------------------*/
//###########################################################################
// SDCARD WRITE for TI DSP F28377D
// EDITOR: Juhwan Lee, Konkuk University, GUNACO LAB.
// Date: Feb.20 2019
//###########################################################################
...
FlagStatus boundary_flag = RESET;
FlagStatus Multiblock_flag = RESET;


//*******************************Timer debug 관련변수
extern Uint32 Tim0Flag;
extern Uint32 Tim1Flag;
...
//        if (disk_ioctl(fs->drive, GET_SECTOR_COUNT, 0) != RES_OK)
            res = FR_DISK_ERR;
    }
...
FRESULT f_mount ( BYTE vol,     /* Logical drive number to be mounted/unmounted */
    ...
    if(vol >= _DRIVES)  return FR_INVALID_DRIVE;
...
FRESULT f_write(FIL *fp,        /* Pointer to the file object */
            ...
            UINT *bw){      /* Pointer to number of bytes written */
                    ...
                    *bw = 0;
                    ...
                    /* Multiblock for 2byte! */
                        if(Multiblock_flag){
                            fp->fptr += wcnt;
                            DEF_RESETFLAG(Multiblock_flag);
                        }
                ...
                ABORT(fp->fs, FR_DISK_ERR);
                            ...
                            if (fp->flag & FA__DIRTY) {     /* Write back data buffer prior to following direct transfer */
                                ...
                                DEF_SETFLAG(Q_data_write);

                                if (disk_write(fp->fs->drive, fp->buf, fp->dsect, 1) != RES_OK)
                                    ABORT(fp->fs, FR_DISK_ERR);
                                fp->flag &= ~FA__DIRTY;

                                DEF_RESETFLAG(Q_data_write);
                            ...
                            if (!sect) ABORT(fp->fs, FR_INT_ERR);
                            sect += fp->csect;
                            cc = btw / (SS(fp->fs)/2);                  /* When remaining bytes >= sector size, */
                ...
                if (fp->dsect - sect < cc) {        /* Refill sector cache if it gets dirty by the direct write */
                                    mem_cpy(fp->buf, wbuff + ((fp->dsect - sect) * (SS(fp->fs)/2)), SS(fp->fs)/2);
                                    fp->flag &= ~FA__DIRTY;
                }
#endif
                fp->csect += (BYTE)cc;              /* Next sector address in the cluster */
                wcnt = (SS(fp->fs)/2) * cc;             /* Number of bytes transferred */
                DEF_SETFLAG(Multiblock_flag);
                continue;
                            }
#if _FS_TINY
                ...
                fp->fs->winsect = sect;
                        ...
                        wcnt = SS(fp->fs)/2 - (fp->fptr % (SS(fp->fs)/2));    /* Put partial sector into file I/O buffer */
                        if (wcnt > btw) {
                            wcnt = btw;
                        }
                        else{
                            DEF_SETFLAG(boundary_flag);//DSP관련 SD카드 저장관련 최적화 핵심부분
                        }
                        ...
                        fp->fs->wflag = 1;
#else
                        mem_cpy(&fp->buf[fp->fptr % (SS(fp->fs)/2)], wbuff, wcnt);  /* Fit partial sector */
                        if(boundary_flag){
                            fp->fptr = fp->fptr + 256;
                            DEF_RESETFLAG(boundary_flag);//DSP관련 SD카드 저장관련 최적화 핵심부분
                        }
