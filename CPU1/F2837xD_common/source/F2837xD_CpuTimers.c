...
void ConfigCpuTimer(struct CPUTIMER_VARS *Timer, float Freq, float Period)
{
    ...
    //
    // Set pre-scale counter to divide by 예시 20000 (SYSCLKOUT): 분해능 설정! 200E6/20000 = 10000...100us의 분해능을 가지는 Timer 생성
    //

    //프로세서가 200MHZ이면 다음과 같이 자동으로 설정된다. 아닐경우 주의!!
    Div_value = 2000 - 1;

    Timer->RegsAddr->TPR.all  = (Div_value & 0x00FF);
    Timer->RegsAddr->TPRH.all  = (Div_value >> 8);

//    Timer->RegsAddr->TPR.all  = 0x0000;
//    Timer->RegsAddr->TPRH.all  = 0x0000;

    //
    // Counter decrements PRD+1 times each period
    //
    Timer->RegsAddr->PRD.all = temp/(Div_value+1) - 1;
